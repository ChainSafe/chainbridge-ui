...
describe("Example", () => {
  ...
  it("should be valid address", async () => {
    assert.isAddress(
      limeFactoryInstance.contractAddress,
  ...
  it("should create lime", async () => {
    const createTransaction = await limeFactoryInstance.createLime(
      "newLime",
      6,
      8,
      2
  ...
  it("should revert if try to create lime with 0 carbohydrates", async () => {
    let carbohydrates = 0;
    await assert.revert(
      limeFactoryInstance.createLime("newLime2", carbohydrates, 8, 2),
  ...
  it("should create lime from another account", async () => {
    ...
    const transaction = await limeFactoryInstance
      .from(bobsAccount /* Could be address or just index in accounts like 4 */)
      .createLime("newLime3", 6, 8, 2);
    // check sender
    assert.equal(
      transaction.from,
      bobsAccount.address,
      "The account that created lime was not bobs"
    );
  ...
  it("should emit event", async () => {
    ...
    await assert.emit(
      limeFactoryInstance.createLime("newLime", 6, 8, 2),
      expectedEvent
    );
  });
